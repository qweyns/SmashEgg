...
class CommandHandlerTest {
    ...
    void infoShowsTheGlobalRulesWithoutArguments() {
        ...
        PlaceholderWords words = plugin.settings().words();
        assertEquals("4.0.0", context.getValue().get("version"));
        assertEquals(words.unset(), context.getValue().get("world"));
        assertEquals(words.unset(), context.getValue().get("entity"));
        assertEquals("30", context.getValue().get("break"));
        assertEquals("70", context.getValue().get("ground"));
        assertEquals(words.spawner(true), context.getValue().get("spawner"));
        assertEquals(words.creative(false), context.getValue().get("creative"));
        assertEquals(words.filter(plugin.settings().filterMode()), context.getValue().get("filter"));
    ...
    void singleArgumentIsAWorldWhenSuchWorldIsLoaded() {
        ...
        verify(plugin).message(eq(sender), eq("info"), context.capture());
        assertEquals("WORLD_NETHER", context.getValue().get("world"));
        assertEquals(plugin.settings().words().unset(), context.getValue().get("entity"));
