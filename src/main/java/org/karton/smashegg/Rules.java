 ...
 */
record Rules(boolean breakOnSpawner, int breakChance, int groundChance, boolean affectCreative) {

    /** Partial override; a {@code null} field means "inherit from the less specific level". */
    record Overrides(Boolean breakOnSpawner, Integer breakChance, Integer groundChance, Boolean affectCreative) {
        boolean isEmpty() {
            return breakOnSpawner == null && breakChance == null && groundChance == null && affectCreative == null;
        }
    }

    Rules apply(Overrides override) {
        return new Rules(
