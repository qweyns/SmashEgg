...
public final class CommandHandler implements CommandExecutor, TabCompleter {
    ...
    private void info(CommandSender sender, String[] args) {
        ...
        String world = senderWorld(sender, words.unset());
        String entity = words.unset();
        if (args.length >= 3) {
            world = args[1];
        ...
        plugin.message(sender, "info", Map.of(
                ...
                "world", world,
                "entity", entity,
                "spawner", words.spawner(rules.breakOnSpawner()),
                "break", Integer.toString(rules.breakChance()),
                "ground", Integer.toString(rules.groundChance()),
                "creative", words.creative(rules.affectCreative()),
                "filter", words.filter(settings.filterMode()),
                "cooldown", Integer.toString(settings.cooldownTicks())));
    }

    /** The world the sender stands in; the console has none, so it gets the configured unset marker. */
    private static String senderWorld(CommandSender sender, String unset) {
        return sender instanceof Player player ? player.getWorld().getName() : unset;
    }
